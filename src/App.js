...
  // --- START: Image Processing State Variables ---
  const [originalImage, setOriginalImage] = useState(null); // Stores the file object for original image
  const [originalImagePreview, setOriginalImagePreview] = useState(null); // Stores URL for local preview of original image
  const [processedImage, setProcessedImage] = useState(null); // Stores URL for displaying processed image
  const [currentImageData, setCurrentImageData] = useState(null); // Stores Base64 string of the image currently being manipulated
  const [message, setMessage] = useState(''); // User feedback messages
  const [isLoading, setIsLoading] = useState(false); // Loading state for API calls
  const [backgroundImage, setBackgroundImage] = useState(null); // For background image file object
  // --- END: Image Processing State Variables ---
...
      try {
        // *** IMPORTANT CHANGE HERE: Update endpoint to /api/upload ***
        const response = await fetch(`/api/upload`, { // Changed from /api to /api/upload
          method: 'POST',
          body: formData,
        });
...
  // Note: This function is mostly fine as the endpoints are already matching
  // the new server routes (/api/remove-background, /api/edit-background, /api/resize-image)
  const makeRealApiCall = async (endpoint, data) => {
    if (!currentImageData && endpoint !== '/api/upload') { // Updated check
...
    // For /api/edit-background when applying an image, we still need FormData
    // For other calls, we can use JSON.
    if (endpoint === '/api/edit-background' && data.background_image) {
      bodyToSend = new FormData();
      if (currentImageData) {
        bodyToSend.append('image_data', currentImageData);
      }
      bodyToSend.append('background_image', data.background_image);
    } else {
...
        method: 'POST',
        headers: headers, // Apply headers if not FormData
        body: bodyToSend,
      });
...
  // ... handleApplyImageBackground (no change needed in its call to makeRealApiCall) ...
  // The makeRealApiCall function needs to be updated to handle the `background_image`
  // being passed as a File object inside the `data` for the `/api/edit-background` endpoint.
...
      return;
    }
    // Instead of formData directly, pass the file as part of the data object
    // and let makeRealApiCall handle creating FormData when needed.
    await makeRealApiCall('/api/edit-background', {
